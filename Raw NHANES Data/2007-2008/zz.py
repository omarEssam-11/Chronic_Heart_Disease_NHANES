...
# Load 2007-2008 datasets (suffix = _E.xpt)
datasets = {
    'DEMO': pd.read_sas('DEMO_E.xpt'),
    'BMX': pd.read_sas('BMX_E.xpt'),
    'BPX': pd.read_sas('BPX_E.xpt'),
    'BPQ': pd.read_sas('BPQ_E.xpt'),
    'DIQ': pd.read_sas('DIQ_E.xpt'),  # Diabetes questionnaire
    'GLU': pd.read_sas('GLU_E.xpt'),
    'HDL': pd.read_sas('HDL_E.xpt'),
    'MCQ': pd.read_sas('MCQ_E.xpt'),
    'RXQ': pd.read_sas('RXQ_RX_E.xpt'),  # Prescription medications
    'SMQ': pd.read_sas('SMQ_E.xpt'),
    'TCHOL': pd.read_sas('TCHOL_E.xpt'),
    'TRIGLY': pd.read_sas('TRIGLY_E.xpt')  # Contains LDL (LBDLDL)
}

# Extract variables (2007-2008 compatible)
variables = {
...
final_df.to_csv('heart_disease_2007_2008.csv', index=False)
