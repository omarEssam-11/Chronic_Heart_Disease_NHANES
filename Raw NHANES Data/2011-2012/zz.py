...
# Load 2011-2012 datasets (suffix = _G.xpt)
datasets = {
    'DEMO': pd.read_sas('DEMO_G.xpt'),
    'BMX': pd.read_sas('BMX_G.xpt'),
    'BPX': pd.read_sas('BPX_G.xpt'),
    'BPQ': pd.read_sas('BPQ_G.xpt'),
    'DIQ': pd.read_sas('DIQ_G.xpt'),  # Diabetes questionnaire
    'GLU': pd.read_sas('GLU_G.xpt'),
    'HDL': pd.read_sas('HDL_G.xpt'),
    'MCQ': pd.read_sas('MCQ_G.xpt'),
    'RXQ': pd.read_sas('RXQ_RX_G.xpt'),  # Corrected dataset name
    'SMQ': pd.read_sas('SMQ_G.xpt'),
    'TCHOL': pd.read_sas('TCHOL_G.xpt'),
    'TRIGLY': pd.read_sas('TRIGLY_G.xpt')
}

# Extract variables (2011-2012 compatible)
variables = {
    ...
    'RXQ': ['SEQN', 'RXDDRUG', 'RXDDRGID'],  # Use drug name/code
    'SMQ': ['SEQN', 'SMQ020'],
    'TCHOL': ['SEQN', 'LBXTC'],
    'TRIGLY': ['SEQN', 'LBDLDL']
}
...
# Multum Lexicon codes for statins (from RXQ_DRUG docs)
statin_codes = [
    'd00973',  # Atorvastatin
    'd00974',  # Simvastatin
    'd00975',  # Pravastatin
...
final_df.to_csv('heart_disease_2011_2012.csv', index=False)
