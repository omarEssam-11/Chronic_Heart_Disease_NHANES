...
# Load 2013-2014 datasets (suffix = _H.xpt)
datasets = {
    'DEMO': pd.read_sas('DEMO_H.xpt'),
    'BMX': pd.read_sas('BMX_H.xpt'),
    'BPX': pd.read_sas('BPX_H.xpt'),
    'BPQ': pd.read_sas('BPQ_H.xpt'),
    'DIQ': pd.read_sas('DIQ_H.xpt'),
    'GLU': pd.read_sas('GLU_H.xpt'),
    'HDL': pd.read_sas('HDL_H.xpt'),
    'MCQ': pd.read_sas('MCQ_H.xpt'),  # Verified MCQ dataset
    'RXQ': pd.read_sas('RXQ_RX_H.xpt'),
    'SMO': pd.read_sas('SMQ_H.xpt'),   # Smoking dataset (2013-2014 name)
    'TCHOL': pd.read_sas('TCHOL_H.xpt'),
    'TRIGLY': pd.read_sas('TRIGLY_H.xpt')  # Contains LDL (LBDLDL)
}

# Extract variables (2013-2014 compatible)
variables = {
    ...
    'GLU': ['SEQN', 'LBXGLU'],
    'HDL': ['SEQN', 'LBDHDD'],
    'MCQ': ['SEQN', 'MCQ160E', 'MCQ160C'],  # Heart disease variables
    'RXQ': ['SEQN', 'RXDRSC1'],  # Statin use
    'SMO': ['SEQN', 'SMQ020'],    # Smoking variable in SMO_H.xpt
    'TCHOL': ['SEQN', 'LBXTC'],
    'TRIGLY': ['SEQN', 'LBDLDL']
}
...
# Save to CSV
final_df.to_csv('heart_disease_2013_2014.csv', index=False)
